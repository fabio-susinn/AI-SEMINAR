...
def run_simulation(
    ...
    pois: list[POI],
    strategy_name: str = "interests",
    max_steps: int = MAX_STEPS,
    verbose: bool = True,
) -> tuple[list[TouristAgent], POITracker]:
 
    strategy = ScoringFactory.get_strategy(strategy_name)
 
    tracker = POITracker()
    tracker.setup(pois)
    agents = [
        TouristAgent(i, p, pois, tracker=tracker, strategy=strategy)
    ...
    if verbose:
        ...
        print(f"  Avg POIs : {sum(visited)/len(visited):.1f}")
        print(f"  Total visits : {sum(visited)}")
...
def main():
    ...
    parser.add_argument("--agents",       type=int,  default=10,
                        help="Number of tourist agents")
    parser.add_argument("--seed",         type=int,  default=None,
                        help="Random seed for reproducibility")
    parser.add_argument("--workers",      type=int,  default=5,
                        help="Parallel workers for sentiment API calls")
    parser.add_argument("--strategy",     type=str,  default="interests",
                        choices=ScoringFactory.available_strategies(),
                        help="Recommender strategy to evaluate")
    parser.add_argument("--no-sentiment", action="store_true",
                        help="Skip LLM sentiment step")
    parser.add_argument("--verbose",      action="store_true",
                        help="Print per-agent details")
    parser.add_argument("--data",         type=str,  default=str(DATA_FILE),
                        help="Path to POI JSON file")
    ...
    out_dir = RESULTS_DIR / args.strategy   # separate folder per strategy
    out_dir.mkdir(parents=True, exist_ok=True)
 
    if args.no_sentiment:
        print("\nSkipping sentiment analysis (--no-sentiment)")
        with open(out_dir / "agents_no_sentiment.json", "w", encoding="utf-8") as f:
            json.dump([a.summary() for a in agents], f, indent=2)
        print(f"Agent summaries saved to {out_dir}/agents_no_sentiment.json")
        return
    ...
    save_results(results, tracker, out_dir=str(out_dir))
